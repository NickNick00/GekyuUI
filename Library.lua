-- Library.lua
-- GekyuUI - Versão FINAL DEFINITIVA
-- Correções: Drag Externo, Resize fixo, Anti-Clickthrough, Minimizar Oculta Tudo.
...
function Library:CreateWindow(title)
    ...
    self.IsResizing = false -- Flag para impedir conflito de drag

    -- 1. Criação do MainFrame (Ordem correta para evitar erro nil)
    self.MainFrame = Instance.new("Frame")
    self.MainFrame.Active = true -- Bloqueia cliques no jogo
    ...
    BottomDrag.MouseEnter:Connect(function() safeTween(DragIcon, TweenInfo.new(0.25), {BackgroundTransparency = 0.3, BackgroundColor3 = COLORS.Accent}) end)
    BottomDrag.MouseLeave:Connect(function() safeTween(DragIcon, TweenInfo.new(0.25), {BackgroundTransparency = 0.8, BackgroundColor3 = COLORS.TextDim}) end)

    -- 3. Redimensionar (Resize Handle)
    local function updateResize()
        ...
        ResizeHandle.Position = UDim2.new(1, 4, 1, 4) -- Canto externo
        ...
        BlockOverlay.Size = UDim2.new(100, 0, 100, 0)
        BlockOverlay.Position = UDim2.new(-50,0,-50,0)
        BlockOverlay.BackgroundTransparency = 1
        BlockOverlay.Text = ""
        BlockOverlay.Visible = false
        BlockOverlay.ZIndex = 200
        BlockOverlay.Parent = self.MainFrame

        ResizeHandle.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                self.IsResizing = true -- Trava o drag
                resizing = true
        ...
        UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                self.IsResizing = false -- Destrava o drag
                resizing = false
                BlockOverlay.Visible = false
            end
        end)
    ...
    local function setupDrag(dragObject)
        dragObject.InputBegan:Connect(function(input)
            if self.IsResizing then return end -- Se estiver redimensionando, não arrasta
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                dragging = true
                dragStart = input.Position
                startPos = self.MainFrame.Position
                ContextActionService:BindAction("GekyuDrag", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseMovement, Enum.UserInputType.Touch)
                local conn
    ...
    local minimizeBtn = CreateControlButton(TopBar, "−", -102, nil, function()
        ...
        if minimized then
            ...
            local h = self.MainFrame:FindFirstChild("ResizeHandle")
    ...
    function self:ToggleConfigPanel()
        if self.ConfigPanel and self.ConfigPanel.Parent then
            ...
            task.delay(0.35, function() if self.ConfigPanel then self.ConfigPanel:Destroy() self.ConfigPanel = nil end end)
        else
            ...
            configTopBar.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    ContextActionService:BindAction("ConfigDrag", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseMovement, Enum.UserInputType.Touch)
            ...
            UserInputService.InputChanged:Connect(function(input)
                if configDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                    ...
                    safeTween(panel, TweenInfo.new(0.08, Enum.EasingStyle.Linear), {Position = UDim2.new(configStartPos.X.Scale, configStartPos.X.Offset + delta.X, configStartPos.Y.Scale, configStartPos.Y.Offset + delta.Y)})
                end
            end)
    ...
    function self:ShowSwitchHubPopup()
        Library.Popup("Trocar de Hub", "Deseja abrir o Hub de Jogos/Scripts?\n\nIsso vai **fechar automaticamente** o GekyuUI atual.", function() ScreenGui:Destroy() end, function() end)
    end
    ...
    function self:CreateTab(name)
        ...
        content:GetPropertyChangedSignal("CanvasPosition"):Connect(function() if content.Visible then lastScrollPos = content.CanvasPosition end end)

        table.insert(self.tabs, {button = button, textLabel = textLabel, indicator = indicator, content = content, name = name:upper(), lastScrollPos = lastScrollPos})
        ...
        button.Activated:Connect(function()
            ...
            task.delay(0.15, function() safeTween(button, TweenInfo.new(0.15), {Size = UDim2.new(1,-16,0,46)}) end)
            safeTween(button, TweenInfo.new(0.25), {BackgroundColor3 = COLORS.ElementHover})
            self.currentTab = {button = button, content = content, indicator = indicator, textLabel = textLabel, lastScrollPos = lastScrollPos}
        end)
        list:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() content.CanvasSize = UDim2.new(0, 0, 0, list.AbsoluteContentSize.Y + 20) end)
        return content
    end
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if query == "" then
            for _, tab in ipairs(self.tabs) do tab.button.Visible = true end
            return
        end
            ...
            if not match then
                for _, child in ipairs(tab.content:GetChildren()) do
                    if child:IsA("Frame") or child:IsA("TextButton") then
                        ...
                        if lbl and lbl.Text:lower():find(query, 1, true) then match = true break end
                    end
                end
            end
        ...
        if foundAny then
            for _, tab in ipairs(self.tabs) do if tab.button.Visible then tab.button.Activated:Fire() break end end
        end
    end)
    ...
    function Library.Button(parent, text, callback, options)
        ...
        button.MouseEnter:Connect(function() safeTween(button, TweenInfo.new(0.18), {BackgroundColor3 = COLORS.ElementHover}) if icon then safeTween(icon, TweenInfo.new(0.3), {ImageColor3 = COLORS.Accent}) end end)
        button.MouseLeave:Connect(function() safeTween(button, TweenInfo.new(0.18), {BackgroundColor3 = COLORS.Element}) if icon then safeTween(icon, TweenInfo.new(0.3), {ImageColor3 = COLORS.Text}) end end)
        button.Activated:Connect(function()
    ...
    function Library.Toggle(parent, text, default, callback)
        ...
        local function update()
            ...
            safeTween(circle, TweenInfo.new(0.28, Enum.EasingStyle.Back), {Position = state and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 3, 0.5, -10)})
        end
        hitbox.Activated:Connect(function() state = not state update() callback(state) end)
        update()
        return container
    end
    ...
    function Library.ToggleWithCheckboxes(parent, toggleText, checkboxesList, callback)
        ...
        for _, checkName in ipairs(checkboxesList) do
            ...
            checkHitbox.Activated:Connect(function()
                ...
                safeTween(checkBoxVisual, TweenInfo.new(0.18), {BackgroundColor3 = cState and COLORS.Accent or Color3.fromRGB(40, 40, 60)})
            end)
        end
        toggleBtn.Activated:Connect(function()
            ...
            safeTween(container, TweenInfo.new(0.38, Enum.EasingStyle.Quint), {Size = UDim2.new(0.95, 0, 0, finalHeight)})
            callback(state)
        end)
    end
    ...
    function Library.Slider(parent, text, min, max, default, callback)
        ...
        knobArea.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = true updateValue(input) end
        end)
        UserInputService.InputChanged:Connect(function(input)
    ...
    function Library.Dropdown(parent, text, options, defaultIndex, callback)
        ...
        for _, opt in ipairs(options) do
            ...
            btn.Activated:Connect(function() selectedText.Text = limitedOpt callback(opt) toggle() end)
            btn.MouseEnter:Connect(function() safeTween(btn, TweenInfo.new(0.15), {BackgroundTransparency = 0.92}) end)
            btn.MouseLeave:Connect(function() safeTween(btn, TweenInfo.new(0.15), {BackgroundTransparency = 1}) end)
        end
    ...
    function Library.DropdownMulti(parent, text, options, defaultSelected, callback)
        ...
        if defaultSelected then
            for _, v in ipairs(defaultSelected) do for i, opt in ipairs(options) do if opt == v then selected[i] = true break end end end
        end
        local function updatePreview()
            ...
            local selectedList = {}
            for i, isSel in pairs(selected) do if isSel then table.insert(selectedList, options[i]) end end
        ...
        for i, optionName in ipairs(options) do
            ...
            optionBtn.MouseEnter:Connect(function() safeTween(optionBtn, TweenInfo.new(0.15), {BackgroundTransparency = 0.92}) end)
            optionBtn.MouseLeave:Connect(function() safeTween(optionBtn, TweenInfo.new(0.15), {BackgroundTransparency = 1}) end)
            optionBtn.Activated:Connect(function() selected[i] = not selected[i] checkMark.Visible = selected[i] updatePreview() end)
        end
    ...
    function Library.InputNumber(parent, text, min, max, default, step, callback)
        ...
        valueBox:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            if valueBox.TextBounds.X > valueBox.AbsoluteSize.X - 10 then valueBox.Text = currentText:sub(1, #currentText - 1) end
        end)
        ...
        minusBtn.Activated:Connect(function() updateValue(currentValue - step) end)
        plusBtn.Activated:Connect(function() updateValue(currentValue + step) end)
        valueBox.FocusLost:Connect(function(enterPressed)
            if enterPressed then
                local num = tonumber(valueBox.Text)
                if num then updateValue(num) else valueBox.Text = tostring(currentValue) end
            end
        end)
        valueBox:GetPropertyChangedSignal("Text"):Connect(function() local num = tonumber(valueBox.Text) if num then currentValue = num end end)
        updateValue(default)
    end
    ...
    function Library.Notify(message, duration, color)
        ...
        if activeNotifications[message] then
            ...
            data.destroyTime = task.delay(duration, function()
                safeTween(data.frame, TweenInfo.new(0.5), {BackgroundTransparency = 1, Position = UDim2.new(1, 0, 0, -40)})
                task.delay(0.5, function() data.frame:Destroy() activeNotifications[message] = nil end)
            end)
            safeTween(data.frame, TweenInfo.new(0.2), {BackgroundColor3 = color})
            task.delay(0.2, function() safeTween(data.frame, TweenInfo.new(0.4), {BackgroundColor3 = COLORS.Background}) end)
            return
        end
        ...
        activeNotifications[message] = {
            ...
            destroyTime = task.delay(duration, function()
                safeTween(notif, TweenInfo.new(0.5), {BackgroundTransparency = 1, Position = UDim2.new(1, 0, 0, -40)})
                task.delay(0.5, function() notif:Destroy() activeNotifications[message] = nil end)
            end)
        }
    end
    ...
    function Library.Popup(titleText, messageText, onConfirm, onCancel)
        ...
        local function closePopup()
            safeTween(popup, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.In), {BackgroundTransparency = 1, Size = UDim2.new(0, 340, 0, 210)})
            safeTween(overlay, TweenInfo.new(0.28), {BackgroundTransparency = 1})
            task.delay(0.3, function() popupHolder:Destroy() end)
        end
        cancelBtn.Activated:Connect(function() if onCancel then onCancel() end closePopup() end)
        confirmBtn.Activated:Connect(function() if onConfirm then onConfirm() end closePopup() end)
    end
